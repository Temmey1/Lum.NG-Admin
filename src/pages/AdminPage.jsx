...
const inputCls =
  "w-full bg-[var(--input-bg)] border border-[var(--border)] rounded px-3.5 py-2.5 text-[var(--text)] text-sm focus:outline-none focus:border-[var(--gold-dim)] placeholder:text-[var(--text-ghost)] transition-all";
const labelCls =
  "text-[11px] tracking-widest uppercase text-[var(--text-muted)] block mb-1.5";
...
function CategoryCombobox({ value, options, onChange }) {
  ...
  const commitFromInput = () => {
    ...
    if (existing) {
      if (existing.value !== value) onChange(existing.value);
    } else {
      const slug = slugify(trimmed) || slugify(input);
  ...
  const filtered = React.useMemo(() => {
    ...
    if (typed) {
      ...
      const foundExact = options.some(
        (o) =>
          o.label.toLowerCase() === typed.toLowerCase() ||
          o.value.toLowerCase() === slug,
      );
      if (!foundExact && slug) {
        list.unshift({ value: slug, label: `+ Add new: "${typed}"` });
      }
    }
    return list;
  }, [input, options]);
    ...
    <div ref={wrapperRef} className="relative">
      <div className="relative">
            ...
            if (e.key === "ArrowDown") {
              ...
              if (open && filtered[activeIdx]) {
                ...
                const matchingOption = options.find(
                  (o) => o.value === chosen.value,
                );
                setInput(
                  matchingOption
                    ? matchingOption.label
                    : chosen.label.startsWith("+ Add new:")
                      ? input.trim()
                      : chosen.label,
                );
        ...
        <button
          ...
          tabIndex={-1}
          onClick={() => setOpen((o) => !o)}
          className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-[var(--text-ghost)] hover:text-[var(--gold)]"
        >
          {open ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
        </button>
      </div>
      {open && filtered.length > 0 && (
        <ul className="absolute z-30 mt-1 w-full max-h-64 overflow-y-auto rounded-md border border-[var(--border)] bg-[var(--bg-card)] shadow-xl">
          {filtered.map((opt, i) => {
            const isNew = opt.label.startsWith("+ Add new:");
            ...
            return (
              <li
                ...
                className={`cursor-pointer px-3.5 py-2 text-sm border-b last:border-b-0 border-[var(--border)]/50 transition-colors flex items-center justify-between ${
                  i === activeIdx
                    ? "bg-[var(--gold-glow)] text-[var(--gold-light)]"
                    : isNew
                      ? "text-[var(--gold)] hover:bg-[var(--input-bg)]"
...
function StatusBadge({ status }) {
  const map = {
    pending:
      "bg-[rgba(232,169,76,0.15)] text-[var(--gold)] border-[var(--gold-dim)]",
    confirmed:
      "bg-[rgba(76,175,110,0.15)] text-green-400 border-[rgba(76,175,110,0.3)]",
    processing:
      "bg-[rgba(100,130,220,0.15)] text-blue-300 border-[rgba(100,130,220,0.3)]",
    fulfilled:
      "bg-[rgba(76,175,110,0.2)] text-green-300 border-[rgba(76,175,110,0.4)]",
    cancelled:
      "bg-[rgba(232,92,92,0.15)] text-[var(--danger)] border-[rgba(232,92,92,0.3)]",
    active:
      "bg-[rgba(76,175,110,0.2)] text-green-400 border-[rgba(76,175,110,0.3)]",
    oos: "bg-[rgba(232,92,92,0.15)] text-[var(--danger)] border-[rgba(232,92,92,0.2)]",
  };
  return (
    <span
      className={`text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-sm border ${map[status] || map.pending}`}
    >
      {status === "oos"
        ? "Out of Stock"
        : status === "active"
...
export default function AdminPage() {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(
    () => localStorage.getItem("lumng_admin_sidebar_collapsed") === "1",
  );
  ...
  const [notifEnabled, setNotifEnabled] = useState(
    () => localStorage.getItem("lumng_admin_notif_enabled") === "1",
  );
  ...
  const failed = (label, err) =>
    showToast(
      `⚠ ${label} failed: ${err.response?.data?.message || err.message}`,
    );
  ...
  useOrderNotifications({
    ...
    onNewOrder: (order) => {
      ...
      showToast(
        `🛎️ New order — ${order.custName || "a customer"} · ${formatPrice(order.total)}`,
      );
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      failed("Loading site content", err);
    }
  }, []);
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      // Silently ignore — fall back to static list
    }
  }, []);
  ...
  const updateOrderStatusAction = async (ref, status) => {
    try {
      ...
      failed("Update order status", err);
    }
  };
  ...
  /* Merged categories (static + server-loaded custom ones) */
  const mergedCategoryOptions = useMemo(() => {
    const byValue = new Map();
    for (const c of PRODUCT_CATEGORIES) byValue.set(c.value, { ...c });
    for (const s of serverCategories || []) {
      if (!s?.value) continue;
      const existing = byValue.get(s.value);
      if (existing) {
        if (!existing.label && s.label) existing.label = s.label;
      } else {
        byValue.set(s.value, { value: s.value, label: s.label || s.value });
      }
    }
  ...
  const uniqueCustomers = [
    ...new Set(orders.map((o) => o.customer?.email).filter(Boolean)),
  ].length;

  /* Testimonials helpers */
  const updateTestimonial = (i, key, val) =>
    setTestimonialsForm((arr) =>
      arr.map((t, idx) => (idx === i ? { ...t, [key]: val } : t)),
    );
  ...
  const updateCard = (i, key, val) =>
    setFabricCards((arr) =>
      arr.map((c, idx) => (idx === i ? { ...c, [key]: val } : c)),
    );
  const addCard = () =>
    setFabricCards((arr) => [
      ...arr,
      { label: "New Fabric", pattern: "linear-gradient(135deg,#111,#333)" },
    ]);
  const removeCard = (i) =>
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] flex font-[Inter]">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 bg-[var(--bg-2)] border-b border-[var(--border)] flex items-center justify-between px-4 z-30">
        ...
        <div className="flex items-center gap-2">
          <img
            src="/logo.jpeg"
            alt="LUM NG"
            className="w-6 h-6 rounded-full object-cover border border-[var(--gold-dim)]"
          />
          <span className="font-[Playfair_Display] text-sm font-black tracking-widest text-[var(--text)]">
            LUM NG
          </span>
        </div>
        <div className="flex items-center gap-2">
      ...
      >
        <div
          className={`px-6 py-7 border-b border-[var(--border)] flex items-center justify-between ${sidebarCollapsed ? "lg:px-4" : ""}`}
        >
          <div
            className="font-[Playfair_Display] text-xl font-black tracking-widest"
            style={{
              background:
                "linear-gradient(135deg,var(--text),var(--gold-light))",
          ...
          >
            <div className="flex items-center gap-2.5">
              <img
                src="/logo.jpeg"
                alt="LUM NG"
                className="w-8 h-8 rounded-full object-cover border border-[var(--gold-dim)] flex-shrink-0"
              />
        ...
        {!sidebarCollapsed && (
          <div className="text-[10px] tracking-[0.15em] uppercase text-[var(--gold)] px-6 -mt-5 pb-2 hidden lg:block">
            Admin Panel
          </div>
        )}
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5">
          {/* Main group */}
          {!sidebarCollapsed && (
            <div className="text-[10px] tracking-[0.15em] uppercase text-[var(--text-ghost)] px-3.5 mb-2 mt-1 hidden lg:block">
              Main
            </div>
          )}
          ...
          {(websiteOpen || sidebarCollapsed) &&
            NAV.filter((n) => n.group === "website").map(
              ({ id, label, Icon }) => (
                <button
                  key={id}
                  title={sidebarCollapsed ? label : undefined}
          ...
          {/* Config */}
          {!sidebarCollapsed && (
            <div className="text-[10px] tracking-[0.15em] uppercase text-[var(--text-ghost)] px-3.5 mb-2 mt-4 hidden lg:block">
              Config
            </div>
          )}
          <button
          ...
          <a
            href={`${STOREFRONT_URL}/shop`}
            target="_blank"
            rel="noreferrer"
            title={sidebarCollapsed ? "Preview Shop" : undefined}
        ...
        <div
          className={`px-6 py-3 border-t border-[var(--border)] hidden lg:flex flex-col gap-2.5 ${sidebarCollapsed ? "lg:px-0 lg:items-center" : ""}`}
        >
          <div
        ...
        <div
          className={`px-6 py-5 border-t border-[var(--border)] ${sidebarCollapsed ? "lg:px-3" : ""}`}
        >
          {!sidebarCollapsed && (
            <div className="text-[12px] text-[var(--text-muted)] mb-3 hidden lg:block">
              <strong className="text-[var(--text-dim)] block">
                {localStorage.getItem("lumng_admin_username") || "Admin"}
              </strong>
              Logged in
            </div>
          )}
      ...
      {/* ── Main content ── */}
      <main
        className={`flex-1 p-5 lg:p-10 min-h-screen pt-20 lg:pt-10 transition-all duration-300 ${sidebarCollapsed ? "lg:ml-[76px]" : "lg:ml-[260px]"}`}
      >
        {/* ── DASHBOARD ── */}
        {page === "dashboard" && (
          <motion.div
          ...
          >
            <h1 className="font-[Playfair_Display] text-3xl font-bold text-[var(--text)] mb-8">
              Dashboard{" "}
              <span className="text-[14px] font-normal text-[var(--text-muted)] font-[Inter]">
                Welcome back 👋
              </span>
            </h1>
            <div className="grid grid-cols-2 xl:grid-cols-4 gap-5 mb-8">
              {[
                ...
                <div
                  key={label}
                  className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 hover:border-[var(--gold-dim)] transition-all"
                >
                  <div
                    className="flex items-center gap-2 text-[11px] tracking-wider uppercase mb-3"
                    style={{ color }}
                  >
                    <Icon size={14} /> {label}
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-6">
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
                  ...
                  <button
                    ...
                    className="text-[12px] text-[var(--text-muted)] hover:text-[var(--gold-light)] transition-colors"
                  >
                    View All →
                  </button>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[480px]">
                    <thead>
                      <tr>
                        {["Ref", "Customer", "Total", "Status"].map((h) => (
                          <th
                            key={h}
                            className="text-left text-[10px] tracking-wider uppercase text-[var(--gold)] px-4 py-3 border-b border-[var(--border)]"
                          >
                            {h}
                          </th>
                        ))}
                    ...
                    <tbody>
                      {orders.slice(0, 5).map((o) => (
                        <tr key={o.ref} className="hover:bg-[var(--input-bg)]">
                          ...
                          <td className="px-4 py-3 text-[14px] text-[var(--text-dim)]">
                            {o.customer?.name || "—"}
                          </td>
                          <td className="px-4 py-3 text-[14px] text-[var(--text-dim)]">
              ...
              <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-6 flex flex-col gap-3">
                <span className="font-semibold text-[var(--text)] mb-2">
                  Quick Actions
                </span>
                {[
                  ["+ Add New Product", "add"],
                    ...
                    {products.map((p) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          {p.badge && (
                            <span className="ml-2 bg-[var(--gold)] text-[var(--bg)] text-[9px] font-bold tracking-wider uppercase px-1.5 py-0.5 rounded-sm">
                              {p.badge}
                            </span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-sm text-[var(--text-muted)] capitalize">
                          {p.category}
                        </td>
                        <td className="px-4 py-3 text-sm text-[var(--gold-light)]">
                          {formatPrice(p.price)}
                        </td>
                        <td className="px-4 py-3 text-sm text-[var(--text-muted)]">
                          {p.bulkPrice
                            ? `${formatPrice(p.bulkPrice)} (${p.bulkMin}+)`
                            : "—"}
                        </td>
                        <td className="px-4 py-3">
                          <StatusBadge status={p.inStock ? "active" : "oos"} />
                        </td>
                        <td className="px-4 py-3">
              ...
              <div className="grid grid-cols-1 gap-5 max-w-2xl">
                ...
                <Field label="Tags (comma separated)">
                  <input
                    value={form.tags}
                    onChange={setF("tags")}
                    placeholder="traditional, colorful, wax print"
                    className={inputCls}
                  />
                </Field>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <select
                            value={o.status}
                            onChange={(e) =>
                              updateOrderStatusAction(o.ref, e.target.value)
                            }
        ...
        {page === "web-hero" && (
          ...
          >
            <PageHeader
              title="Hero Section"
              sub="Landing page hero — first thing visitors see"
            />
            ...
            >
              <div className="grid grid-cols-1 gap-4">
                <Field label="Eyebrow Text">
                  <input
                    ...
                    onChange={(e) =>
                      setHeroForm((f) => ({ ...f, eyebrow: e.target.value }))
                    }
                    className={inputCls}
                  />
                </Field>
                ...
                <Field label="Subtitle">
                  <textarea
                    ...
                    onChange={(e) =>
                      setHeroForm((f) => ({ ...f, subtitle: e.target.value }))
                    }
                    rows={3}
                    className={inputCls + " resize-y"}
                  />
              ...
              <SaveBar
                onSave={() => {
                  updateSiteSection("hero", heroForm);
                  saved("Hero copy");
                }}
              />
            </SectionCard>
            ...
            >
              <div className="flex flex-col gap-4">
                {fabricCards.map((card, i) => (
                  <div
                    key={i}
                    className="flex gap-3 items-start bg-[var(--bg-3)] border border-[var(--border)] rounded-lg p-4"
                  >
                    <div
                      className="w-16 h-16 rounded-md flex-shrink-0 border border-[var(--border)]"
                    ...
                    <div className="flex-1 grid grid-cols-1 gap-2">
                      ...
                      <Field label="CSS Gradient Pattern">
                        <input
                          value={card.pattern}
                          onChange={(e) =>
                            updateCard(i, "pattern", e.target.value)
                          }
              ...
              <SaveBar
                onSave={() => {
                  updateSiteSection("hero", { ...heroForm, fabricCards });
                  saved("Hero cards");
                }}
              />
            </SectionCard>
        ...
        {page === "web-about" && (
          ...
          >
            <PageHeader
              title="About Section"
              sub="The story section below the hero"
            />
            <SectionCard
              title="About Copy"
              onReset={() => setAboutForm(siteContent.about)}
            >
              <div className="grid grid-cols-1 gap-4">
                <FieldRow>
                  <Field label="Eyebrow Text">
                    <input
                      ...
                      onChange={(e) =>
                        setAboutForm((f) => ({ ...f, eyebrow: e.target.value }))
                      }
                      className={inputCls}
                    />
                  </Field>
                ...
                <FieldRow>
                  <Field label="Title Line">
                    <input
                      ...
                      onChange={(e) =>
                        setAboutForm((f) => ({ ...f, title: e.target.value }))
                      }
                      className={inputCls}
                    />
                  </Field>
                ...
                <Field label="Body Paragraph 1">
                  <textarea
                    ...
                    onChange={(e) =>
                      setAboutForm((f) => ({ ...f, body1: e.target.value }))
                    }
                    rows={3}
                    className={inputCls + " resize-y"}
                  />
                ...
                <Field label="Body Paragraph 2">
                  <textarea
                    ...
                    onChange={(e) =>
                      setAboutForm((f) => ({ ...f, body2: e.target.value }))
                    }
                    rows={3}
                    className={inputCls + " resize-y"}
                  />
            ...
            <SectionCard title="Stats Bar">
              ...
              <SaveBar
                onSave={() => {
                  updateSiteSection("about", aboutForm);
                  saved("About");
                }}
              />
            </SectionCard>
        ...
        {page === "web-marquee" && (
          ...
          >
            <PageHeader
              ...
              sub="The scrolling text strip between hero and about"
            />
            <SectionCard
              title="Ticker Items"
              onReset={() => setMarqueeForm(siteContent.marquee.join(", "))}
            >
              ...
              <p className="text-[12px] text-[var(--text-muted)] mt-2">
                Each item separated by a comma. Dots (·) are added automatically
                between items.
              </p>
              <div className="mt-4 overflow-hidden border border-[var(--border)] rounded-lg py-3 px-4 bg-[var(--bg-3)]">
                <p className="text-[11px] text-[var(--text-ghost)] mb-2 uppercase tracking-wider">
                  Preview:
                </p>
        ...
        {page === "web-fabrics" && (
          ...
          >
            <PageHeader
              title="Fabric Cards — Landing Grid"
              sub="The product cards shown on the homepage. Edit actual products from Products → Manage."
            />
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-7">
              <p className="text-[var(--text-muted)] text-sm leading-relaxed mb-5">
                The landing page fabric grid pulls directly from your{" "}
                <strong className="text-[var(--text-dim)]">Products</strong>{" "}
                list. To control which fabrics show on the homepage, toggle the{" "}
                <strong className="text-[var(--text-dim)]">
                  "Featured on Homepage"
                </strong>{" "}
                flag when adding or editing a product.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {products
                  ...
                  .map((p) => (
                    <div
                      key={p.id}
                      className="flex items-center gap-3 bg-[var(--bg-3)] border border-[var(--border)] rounded-lg p-3"
                    >
                      <ProductThumb
                        product={p}
                      ...
                      <button
                        onClick={() => startEdit(p)}
                        className="text-[var(--text-ghost)] hover:text-[var(--gold-light)] transition-colors"
                      >
                        <Edit size={14} />
                      </button>
                    </div>
                ...
                {products.filter((p) => p.featured).length === 0 && (
                  <p className="col-span-3 text-[var(--text-muted)] text-sm">
                    No featured products yet. Mark products as featured when
                    adding/editing.
                  </p>
                )}
              </div>
        ...
        {page === "web-testimonials" && (
          ...
          >
            <PageHeader
              title="Testimonials"
              sub="Customer reviews shown on the landing page"
            />
            <SectionCard
              title="Reviews"
              onReset={() => setTestimonialsForm(siteContent.testimonials)}
            >
              <div className="flex flex-col gap-4">
                {testimonialsForm.map((t, i) => (
                  <div
                    key={i}
                    className="bg-[var(--bg-3)] border border-[var(--border)] rounded-lg p-5"
                  >
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[12px] text-[var(--gold)] font-semibold">
                        Review #{i + 1}
                      </span>
                      <button
              ...
              <SaveBar
                onSave={() => {
                  setSiteArray("testimonials", testimonialsForm);
                  saved("Testimonials");
                }}
              />
            </SectionCard>
        ...
        {page === "web-cta" && (
          ...
          >
            <PageHeader
              title="CTA Banner"
              sub="The call-to-action strip above the contact section"
            />
            <SectionCard
              title="CTA Copy"
              onReset={() => setCtaForm(siteContent.cta)}
            >
              <div className="grid grid-cols-1 gap-4">
                <Field label="Headline">
                  <input
                    value={ctaForm.title}
                    onChange={(e) =>
                      setCtaForm((f) => ({ ...f, title: e.target.value }))
                    }
                    className={inputCls}
                  />
                </Field>
                <Field label="Subtitle">
                  <input
                    ...
                    onChange={(e) =>
                      setCtaForm((f) => ({ ...f, subtitle: e.target.value }))
                    }
                    className={inputCls}
                  />
                </Field>
                <FieldRow>
                  <Field label="Primary Button">
                    <input
                      value={ctaForm.btnPrimary}
                      onChange={(e) =>
                        setCtaForm((f) => ({
                          ...f,
                          btnPrimary: e.target.value,
                        }))
                      }
                      className={inputCls}
                    />
                  ...
                  <Field label="Secondary Button">
                    <input
                      value={ctaForm.btnSecondary}
                      onChange={(e) =>
                        setCtaForm((f) => ({
                          ...f,
                          btnSecondary: e.target.value,
                        }))
                      }
                      className={inputCls}
                    />
        ...
        {page === "web-contact" && (
          ...
          >
            <PageHeader
              title="Contact Info"
              sub="Displayed in the landing page contact section and footer"
            />
            <SectionCard
              title="Contact Details"
              onReset={() => setContactForm(siteContent.contact)}
            >
              <div className="grid grid-cols-1 gap-4">
                <FieldRow>
                  ...
                  <Field label="Email Address">
                    <input
                      ...
                      placeholder="lumngfabrics@gmail.com"
                      className={inputCls}
                    />
                  </Field>
                </FieldRow>
                ...
                <FieldRow>
                  <Field label="Store Address (pickup)">
                    <input
                      value={contactForm.storeAddress}
                      onChange={(e) =>
                        setContactForm((f) => ({
                          ...f,
                  ...
                  <Field label="Opening Hours">
                    <input
                      ...
                      onChange={(e) =>
                        setContactForm((f) => ({ ...f, hours: e.target.value }))
                      }
              ...
              <SaveBar
                onSave={() => {
                  updateSiteSection("contact", contactForm);
                  saved("Contact Info");
                }}
              />
            </SectionCard>
        ...
        {page === "web-footer" && (
          ...
          >
            ...
            <SectionCard
              ...
              onReset={() => setFooterForm(siteContent.footer)}
            >
              <div className="grid grid-cols-1 gap-4">
                <Field label="Tagline / Description">
                  <textarea
                    ...
                    onChange={(e) =>
                      setFooterForm((f) => ({ ...f, tagline: e.target.value }))
                    }
                    rows={2}
                    className={inputCls + " resize-y"}
                  />
              ...
              <SaveBar
                onSave={() => {
                  updateSiteSection("footer", footerForm);
                  saved("Footer");
                }}
              />
            </SectionCard>
        ...
        {page === "web-seo" && (
          ...
          >
            <PageHeader
              title="SEO / Meta Tags"
              sub="Controls browser tab title and search engine preview"
            />
            <SectionCard
              title="Meta Data"
              onReset={() => setSeoForm(siteContent.seo)}
            >
              <div className="grid grid-cols-1 gap-4">
                <Field label="Site Title (browser tab)">
                  <input
                    ...
                    onChange={(e) =>
                      setSeoForm((f) => ({ ...f, siteTitle: e.target.value }))
                    }
                    placeholder="LUM NG — Unisex Fabric Store | Ilorin, Kwara"
                    className={inputCls}
                  />
                </Field>
                ...
                <div className="bg-[var(--bg-3)] border border-[var(--border)] rounded-lg p-4 mt-2">
                  <p className="text-[10px] uppercase tracking-wider text-[var(--text-ghost)] mb-2">
                    Google Preview
                  </p>
                  <p className="text-blue-400 text-[14px] font-medium">
                    {seoForm.siteTitle ||
                      "LUM NG — Unisex Fabric Store | Ilorin, Kwara"}
                  </p>
                ...
                <p className="text-[12px] text-[var(--text-muted)]">
                  Note: After saving, inform your developer i.e me Timothy lol
                  to update your{" "}
                  <code className="bg-[var(--bg-3)] px-1 rounded">
                    index.html
                  </code>{" "}
                  &lt;title&gt; and &lt;meta name="description"&gt; tags to
                  apply these values.
                </p>
              </div>
              <SaveBar
                onSave={() => {
                  updateSiteSection("seo", seoForm);
                  saved("SEO / Meta");
                }}
              />
            </SectionCard>
          ...
          >
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 sm:p-8 max-w-2xl mb-6">
              ...
              <p className="text-[13px] text-[var(--text-muted)] mb-5 leading-relaxed">
                Get a browser notification the moment a new order comes in — a
                stand-in for now while full email notifications are still being
                finished. Works as long as this dashboard is open in a browser
                tab (it doesn't need to be the active tab, but the browser needs
                to be running).
              </p>
              ...
              {!isNotificationSupported() ? (
                ...
                <div className="text-[13px] text-[var(--danger)] bg-[rgba(232,92,92,0.1)] border border-[rgba(232,92,92,0.2)] rounded px-4 py-3">
                  ...
                  browser's site settings (usually the padlock icon next to the
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-2 text-[13px] text-[var(--text-dim)]">
                    ...
                    {notifEnabled
                      ...
                      : "Permission granted, but notifications are paused"}
                  </div>
                  <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                <Field label="New Username">
                  <input
                    value={newCreds.user}
                    onChange={(e) =>
                      setNewCreds((c) => ({ ...c, user: e.target.value }))
                    }
                ...
                <Field label="New Password">
                  <input
                    ...
                    value={newCreds.pass}
                    onChange={(e) =>
                      setNewCreds((c) => ({ ...c, pass: e.target.value }))
                    }
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-5 sm:p-8 max-w-2xl">
              ...
              <p className="text-sm text-[var(--text-muted)] mb-5">
                Resets all website text/copy to the original defaults. Products
              ...
              <button
                onClick={async () => {
                  if (confirm("Reset all website content to defaults?")) {
                    ...
                    showToast("✓ Website content reset");
                  }
                }}
        ...
        {viewOrder && (
          ...
          >
            ...
            >
              ...
              <h3 className="font-[Playfair_Display] text-xl font-bold text-[var(--text)] mb-1">
                Order Details
              </h3>
              <div className="flex items-center gap-2 mb-6">
                <span className="font-mono text-[13px] text-[var(--gold)]">
                  {viewOrder.ref}
                </span>
                  ...
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(
                        `${STOREFRONT_URL}/order/${viewOrder.publicToken}`,
                      );
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 text-sm">
                {[
                  ...
                  ["Delivery", viewOrder.delivery],
                ].map(([k, v]) => (
                  <div key={k}>
                    <span className="text-[10px] tracking-wider uppercase text-[var(--gold)] block mb-1">
                      {k}
                    </span>
              ...
              <div className="border-t border-[var(--border)] pt-4 flex flex-col gap-2 mb-4">
                {(viewOrder.items || []).map((item, i) => {
                  ...
                  if (!p) return null;
                  const price =
                    item.qty >= (p.bulkMin || Infinity) ? p.bulkPrice : p.price;
                  return (
                    <div
                      key={i}
